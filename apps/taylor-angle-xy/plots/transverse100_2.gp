...
if ( exists("resultsPath") && exists("deployPath") && exists("appPath") ) {
    ...
    exportPath = deployPath."/plots_transverse100_2"
	...
	do for [i_p=1:words(ps)] {
		...
		plot \
       word(files,1) every ::3 using 2:4 with lines lc rgb "black" lw 6 title "analytic", \
       for [i=1:words(files)] word(files,i) every ::3 using 2:3 with lines title word(execnames,i)
	    }
	}
    }
} else {
